...
import { loadExistingConfig, gatherConfig, preservedHandEdits } from '../../src/cli/commands/install.ts';
...
// The OTHER half of preservation: keys the wizard schema does NOT manage (federation, Tide,
// Quartermaster, reader pool, …) must survive a reinstall — the file header promises "Safe to hand-edit".
test('preservedHandEdits keeps hand-added keys (federation/Tide/QM) and drops managed keys + header', () => {
  write([
    '# Captain Memo worker — autogenerated by `captain-memo install`. Safe to hand-edit.',
    'CAPTAIN_MEMO_DATA_DIR=/home/u/.captain-memo',          // managed → dropped (schema re-emits)
    'CAPTAIN_MEMO_EMBEDDER_API_KEY=pa-secret',              // managed → dropped
    'CAPTAIN_MEMO_WORKER_THREADED=1',                       // hand-added → kept
    'CAPTAIN_MEMO_RECALL_AUDIT=1',                          // hand-added → kept
    'CAPTAIN_MEMO_FEDERATION_LISTEN=0',                     // hand-added → kept
    'CAPTAIN_MEMO_FEDERATION_DIAL=1',                       // hand-added → kept
    '# A7 Tide tiering shadow-test — reversible via TIERING=0',
    'CAPTAIN_MEMO_TIDE_TIERING=1',                          // hand-added → kept WITH its comment
  ]);
  const kept = preservedHandEdits(envPath);
  // managed keys + the autogenerated header are NOT preserved (the fresh schema re-emits them)
  expect(kept.some(l => l.includes('CAPTAIN_MEMO_DATA_DIR'))).toBe(false);
  expect(kept.some(l => l.includes('CAPTAIN_MEMO_EMBEDDER_API_KEY'))).toBe(false);
  expect(kept.some(l => l.startsWith('# Captain Memo worker'))).toBe(false);
  // every hand-added key survives
  expect(kept).toContain('CAPTAIN_MEMO_WORKER_THREADED=1');
  expect(kept).toContain('CAPTAIN_MEMO_RECALL_AUDIT=1');
  expect(kept).toContain('CAPTAIN_MEMO_FEDERATION_LISTEN=0');
  expect(kept).toContain('CAPTAIN_MEMO_FEDERATION_DIAL=1');
  expect(kept).toContain('CAPTAIN_MEMO_TIDE_TIERING=1');
  // the explanatory comment travels WITH its key
  expect(kept).toContain('# A7 Tide tiering shadow-test — reversible via TIERING=0');
  const tideIdx = kept.indexOf('CAPTAIN_MEMO_TIDE_TIERING=1');
  expect(kept[tideIdx - 1]).toBe('# A7 Tide tiering shadow-test — reversible via TIERING=0');
});

test('preservedHandEdits returns [] for a missing file', () => {
  expect(preservedHandEdits(join(dir, 'nope.env'))).toEqual([]);
});
