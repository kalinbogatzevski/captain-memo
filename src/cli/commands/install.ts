...
const MANAGED_ENV_KEYS = new Set<string>([
  ...
  'CAPTAIN_MEMO_SUMMARIZER_PROVIDER', 'CAPTAIN_MEMO_SUMMARIZER_MODEL', 'ANTHROPIC_API_KEY',
...
/**
 * Hand-added lines from an existing worker.env that the wizard schema does NOT manage — returned so a
 * reinstall re-appends them instead of silently dropping them. Each preserved KEY=VALUE keeps the
 * contiguous comment lines directly above it (their explanation). Managed keys + the autogenerated
 * header are skipped (the fresh schema re-emits those). Returns [] when the file is absent/unreadable.
 */
export function preservedHandEdits(envPath: string): string[] {
  if (!existsSync(envPath)) return [];
  let raw: string;
  ...
  for (const line of raw.split('\n')) {
    ...
    if (trimmed === '') { comments = []; continue; }          // blank line resets comment grouping
    if (trimmed.startsWith('#')) { comments.push(line); continue; }
    const eq = line.indexOf('=');
    const key = eq > 0 ? line.slice(0, eq).trim() : '';
    if (key && !MANAGED_ENV_KEYS.has(key)) out.push(...comments, line);   // keep its explanation with it
    comments = [];
  }
  return out;
}

/** Full worker.env body: the wizard-managed lines + any hand-added keys preserved from the existing file
 *  (re-appended under a labelled section). Shared by the Linux + Windows install paths. `kept` counts the
 *  preserved KEY=VALUE lines (for the install log). */
function composeWorkerEnvBody(lines: string[], existingEnvPath: string): { body: string; kept: number } {
  const preserved = preservedHandEdits(existingEnvPath);
  const full = preserved.length > 0
    ? [...lines, '', '# --- preserved on reinstall (hand-added keys the install wizard does not manage) ---', ...preserved]
    : lines;
  const kept = preserved.filter(l => !l.trim().startsWith('#')).length;
