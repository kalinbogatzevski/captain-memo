...
//   0. $CLAUDE_CODE_OAUTH_TOKEN env override — the recommended path for a long-lived
//      token (~1 year), generated by `claude setup-token`. Set this in worker.env for a
//      durable daemon: the file token below expires ~24h and Claude Code does NOT
//      auto-refresh it (upstream: anthropics/claude-code#42904, #21765), so a service
//      that only reads the file goes dark a day after the operator's last `claude login`.
//   1. ~/.claude/.credentials.json — the file `claude login` writes on EVERY platform,
//      Windows included (VERIFIED against Claude Code's auth docs — it is a plain JSON
//      file at %USERPROFILE%\.claude\.credentials.json, NOT the Windows Credential
//      Manager). Shape: { claudeAiOauth: { accessToken, refreshToken, expiresAt, … } }.
//      Path is homedir()-based, so a service whose homedir() differs from the logged-in
//      user's (some Windows Scheduled-Task contexts) won't find it — use #0 there.
//
// On 401 we surface a clear "your OAuth token has expired — run `claude
// login` to refresh, or set CLAUDE_CODE_OAUTH_TOKEN" error so the operator knows what to do.
