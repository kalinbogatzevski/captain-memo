...
2. **Ingest validation** — the worker's `ObservationEnqueueSchema` (Zod) gains `origin_agent: z.enum([...ORIGIN_AGENTS]).optional().catch(undefined)`; an absent or non-conforming value is simply omitted, never a 400. (Plain `.optional()` alone only tolerates *absence* — a present-but-invalid value still fails the whole request; `.catch(undefined)` is what actually delivers the "never a 400" guarantee.)
