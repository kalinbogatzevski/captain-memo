# aelita-mcp v1 — Plan 3: Migration + Federation + Optimization + Eval
...
**Predecessor plans:**
- Plan 1 (foundation, **shipped**): `~/projects/aelita-mcp/docs/plans/2026-05-06-aelita-mcp-v1-plan-1-foundation.md`
- Plan 2 (hooks + observation pipeline + summarizer, **drafted in parallel**): `~/projects/aelita-mcp/docs/plans/2026-05-07-aelita-mcp-v1-plan-2-hooks-observations.md` (assumed shipped before Plan 3 starts).

**Goal:** finish the v1 surface area. Add the long-tail features that turn `aelita-mcp` from "working MCP plugin" into "open-source-ready successor to claude-mem":

1. **Migration** from claude-mem (`~/.claude-mem/claude-mem.db`) into the aelita-mcp corpus — read-only, idempotent, claude-mem stays on disk forever.
...
7. **`aelita-mcp doctor`** — green/yellow/red diagnostic of every dependency.
...
| Haiku 4.5/4.6 summarizer + `claude-haiku-*` model wiring | Plan 2 |
...
~/projects/aelita-mcp/
├── bin/
│   └── aelita-mcp                          # (existing) extended with new commands
├── scripts/
...
```
   ...
   └──────────────────────────────────────────────────────────────────┘
                       ...
                       │ stdio MCP                    │ HTTP/MCP
                       │                              │
                Claude Code                  Remote MCPs (e.g. aelita-knowledge,
...
**Note on `created_at_epoch`:** claude-mem stores **milliseconds** (e.g. `1770566467173`). aelita-mcp's MetaStore uses **seconds** (`Math.floor(Date.now() / 1000)`). The transform in Task 2 normalizes by dividing by 1000.
...
```bash
./bin/aelita-mcp inspect-claude-mem
```
...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'aelita-mig-'));
...
export async function migrateFromClaudeMemCommand(args: string[]): Promise<number> {
  ...
  const embedder = new VoyageEmbedder({
    endpoint: process.env.AELITA_MCP_VOYAGE_ENDPOINT ?? DEFAULT_VOYAGE_ENDPOINT,
    model: process.env.AELITA_MCP_VOYAGE_MODEL ?? 'voyage-4-nano',
    ...(process.env.AELITA_MCP_VOYAGE_API_KEY && {
      apiKey: process.env.AELITA_MCP_VOYAGE_API_KEY,
    }),
  });
...
```bash
./bin/aelita-mcp migrate-from-claude-mem --dry-run --limit 5
```
...
beforeAll(() => {
  workDir = mkdtempSync(join(tmpdir(), 'aelita-mig-e2e-'));
  metaPath = join(workDir, 'meta.sqlite3');
...
aelita-mcp inspect-claude-mem

# Real migration (writes to ~/.aelita-mcp/, never to ~/.claude-mem/):
aelita-mcp migrate-from-claude-mem --project erp-platform

# Resumable / partial:
aelita-mcp migrate-from-claude-mem --limit 1000        # process first 1000 rows then stop
aelita-mcp migrate-from-claude-mem --from-id 12000     # resume from observation id
aelita-mcp migrate-from-claude-mem --dry-run           # report only, no writes
```

**Safety contract:**
- `~/.claude-mem/claude-mem.db` is opened with `mode=ro` and never written to or deleted.
- The migration is idempotent: a `migration_progress` table in `~/.aelita-mcp/meta.sqlite3` tracks every `(source_kind, source_id)` pair processed. Re-running picks up only new rows.
- Rollback: drop `~/.aelita-mcp/` and reinstall — claude-mem keeps working independently.
...
beforeEach(() => {
  outDir = mkdtempSync(join(tmpdir(), 'aelita-memmd-w-'));
});
afterEach(() => {
...
export async function transformMemoryMdCommand(args: string[]): Promise<number> {
  ...
  if (!inPath || !outDir) {
    console.error('Usage: aelita-mcp transform-memory-md --in <MEMORY.md> --out <dir> [--dry-run]');
    return 2;
  }
...
mkdir -p /tmp/memout
./bin/aelita-mcp transform-memory-md --in /tmp/in.md --out /tmp/memout
ls /tmp/memout
```
...
test('round-trip — parse → write → parse-again is stable', () => {
  ...
  const outDir = mkdtempSync(join(tmpdir(), 'aelita-memmd-rt-'));
  try {
...
test('round-trip — REAL MEMORY.md is never modified by the test', () => {
  ...
  const outDir = mkdtempSync(join(tmpdir(), 'aelita-memmd-readonly-'));
  try {
...
**Goal:** opt-in, per-project remote knowledge sources. The most concrete example is the `aelita-knowledge` MCP that serves the ERP knowledge base. Generic enough to plug in any future MCP/HTTP backend.

**Per-project config lives at:** `<project_root>/.claude/aelita-mcp.json` (matches Spec Appendix A).
...
beforeEach(() => {
  projectDir = mkdtempSync(join(tmpdir(), 'aelita-fed-cfg-'));
});
afterEach(() => {
...
test('FederationConfigSchema — accepts a complete config', () => {
  ...
  expect(parsed.federation[0]!.name).toBe('aelita-kb');
});
...
test('loadFederationConfig — reads .claude/aelita-mcp.json', () => {
  ...
  writeFileSync(join(projectDir, '.claude/aelita-mcp.json'), JSON.stringify(cfg));
  ...
  expect(loaded.federation[0]!.name).toBe('aelita-kb');
});

test('loadFederationConfig — invalid JSON fails loudly', () => {
  mkdirSync(join(projectDir, '.claude'));
  writeFileSync(join(projectDir, '.claude/aelita-mcp.json'), '{broken');
...
export function loadFederationConfig(projectRoot: string): FederationConfig {
  const file = join(projectRoot, '.claude', 'aelita-mcp.json');
  if (!existsSync(file)) {
...
git commit -m "feat(federation): zod-validated config schema + .claude/aelita-mcp.json loader"
```
...
export class McpFederationClient {
  ...
  private async ensureConnected(): Promise<void> {
    ...
    this.client = new Client(
      { name: 'aelita-mcp-federation', version: '0.1.0' },
      { capabilities: {} },
    );
...
export class CircuitBreaker {
  ...
  /** Inspection helper for `aelita-mcp federation status`. */
...
The worker now reads federation config from the project root (resolved via `AELITA_MCP_PROJECT_ROOT` env var, falling back to `cwd`), instantiates clients, and merges federated hits with local hits in `/search/all`.
...
const projectRoot = process.env.AELITA_MCP_PROJECT_ROOT ?? process.cwd();
...
### Task 19: `aelita-mcp federation status` CLI command
...
export async function federationCommand(args: string[]): Promise<number> {
  ...
  if (snap.length === 0) {
    ...
    console.log('Add them via <project>/.claude/aelita-mcp.json.');
    return 0;
  }
...
```bash
./bin/aelita-mcp federation status
```

Expected: prints "No federation remotes configured" if no `.claude/aelita-mcp.json` exists, otherwise per-remote lines.
...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'aelita-opt-'));
...
let store: MetaStore;
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'aelita-audit-'));
...
- A CLI command `aelita-mcp eval` (Task 28) that runs against the live worker.
...
The CI-friendly form is `aelita-mcp eval --set <file> --baseline <file>`. Exit code 1 if recall@5 drops by more than 5 percentage points vs. the baseline.
...
```json
{
  "name": "aelita-mcp default eval set",
...
### Task 28: `aelita-mcp eval` CLI command + regression detector
...
export async function evalCommand(args: string[]): Promise<number> {
  ...
  if (!setPath) {
    console.error('Usage: aelita-mcp eval --set <file> [--baseline <file>] [--update-baseline]');
    return 2;
  }
...
test('eval — end-to-end against a tiny worker corpus', async () => {
  const workDir = mkdtempSync(join(tmpdir(), 'aelita-eval-'));
...
**Idempotence:** re-running just verifies + restarts the service. `--uninstall` tears it down without touching `~/.aelita-mcp/`.
...
#
# One-shot installer for a local Voyage embeddings endpoint on the aelita-mcp dev box.
...
# Pass --uninstall to remove. Never touches ~/.aelita-mcp/ data.

set -u
# NOTE: deliberately not setting -e — every check is explicit so we can give a useful error.

PORT="${VOYAGE_PORT:-8124}"
SERVICE_NAME="${VOYAGE_SERVICE_NAME:-aelita-voyage}"
INSTALL_DIR="${VOYAGE_INSTALL_DIR:-/opt/aelita-voyage}"
ARTIFACT_URL="${VOYAGE_ARTIFACT_URL:-}"   # required for fresh install
RUN_USER="${VOYAGE_RUN_USER:-aelita-voyage}"
...
Environment overrides:
  ...
  VOYAGE_SERVICE_NAME  (default aelita-voyage)
  VOYAGE_INSTALL_DIR   (default /opt/aelita-voyage)
  VOYAGE_ARTIFACT_URL  (required on first install — fetched into INSTALL_DIR)
  VOYAGE_RUN_USER      (default aelita-voyage; created if missing)
EOF
}
...
[Unit]
Description=aelita-mcp local Voyage embeddings endpoint
...
cmd_uninstall() {
  ...
  warn "~/.aelita-mcp/ data is NEVER touched by this script."
}
...
# 3. Healthcheck via aelita-mcp
aelita-mcp doctor                           # Voyage check should be PASS

# 4. Uninstall (does NOT touch ~/.aelita-mcp/)
sudo ./scripts/install-voyage.sh --uninstall
./scripts/install-voyage.sh --status        # expect: not installed

# 5. Verify ~/.aelita-mcp/ is untouched
ls ~/.aelita-mcp/                           # expect: meta.sqlite3 + vector-db still there
```

**macOS fallback:** the script targets Linux. On macOS, run a containerized Voyage instance manually:

```bash
docker run -d --name aelita-voyage -p 127.0.0.1:8124:8124 <voyage-image>
```

Then point `AELITA_MCP_VOYAGE_ENDPOINT=http://localhost:8124/v1/embeddings` at it.
```
...
### Task 33: `aelita-mcp doctor` — diagnostic command
...
3. **Voyage endpoint** — POST a 1-token embedding probe (or skip with WARN if `AELITA_MCP_VOYAGE_API_KEY` not set and the endpoint demands auth).
...
6. **Disk usage** — `~/.aelita-mcp/` size; WARN > 5GB, FAIL > 50GB (Spec §5).
...
async function checkStats(): Promise<Check> {
  try {
    ...
    return { name: 'vector-db', status: 'FAIL',
             ...
             remediation: 'check ~/.aelita-mcp/vector-db/embeddings.db permissions' };
  }
}

async function checkVoyage(): Promise<Check> {
  const endpoint = process.env.AELITA_MCP_VOYAGE_ENDPOINT ?? DEFAULT_VOYAGE_ENDPOINT;
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 1500);
    const headers: Record<string, string> = { 'content-type': 'application/json' };
    if (process.env.AELITA_MCP_VOYAGE_API_KEY) {
      headers.authorization = `Bearer ${process.env.AELITA_MCP_VOYAGE_API_KEY}`;
    }
...
async function checkFederation(): Promise<Check> {
  try {
    ...
    return { name: 'federation', status: 'WARN',
             ...
             remediation: 'inspect: aelita-mcp federation status' };
  } catch (err) {
    return { name: 'federation', status: 'WARN',
  ...
  try {
    ...
    if (failed > 0) {
      return { name: 'observation-queue', status: 'WARN',
               ...
               remediation: 'inspect Plan-2 queue logs in ~/.aelita-mcp/logs' };
    }
    if (pending > 100) {
      return { name: 'observation-queue', status: 'WARN',
...
function checkDisk(): Check {
  ...
  if (mb > 50000) {
    return { name: 'disk', status: 'FAIL', detail: `${mb.toFixed(0)} MB in ${DATA_DIR}`,
             remediation: 'consider purge: aelita-mcp purge --before <date> --yes' };
  }
  if (mb > 5000) {
    return { name: 'disk', status: 'WARN', detail: `${mb.toFixed(0)} MB in ${DATA_DIR}`,
             remediation: 'optional: aelita-mcp optimize detect to surface duplicates' };
  }
...
```bash
aelita-mcp doctor                              # diagnose every dependency
aelita-mcp status / stats                      # worker health + corpus stats

# Indexing
aelita-mcp reindex                             # cheap sha-diff reindex
aelita-mcp reindex --channel memory --force    # full re-embed of memory channel

# Migration (one-time, read-only on claude-mem)
aelita-mcp inspect-claude-mem                  # zero-risk schema dump
aelita-mcp migrate-from-claude-mem             # full migration
aelita-mcp migrate-from-claude-mem --dry-run --limit 100

# MEMORY.md transformation (one-time)
aelita-mcp transform-memory-md --in MEMORY.md --out memory/

# Federation
aelita-mcp federation status                   # remote breaker + latency

# Memory hygiene
aelita-mcp optimize detect                     # find near-duplicates
aelita-mcp optimize list                       # show top clusters
aelita-mcp optimize merge <cluster_id>         # keep newest, archive rest

# Removals (audited)
aelita-mcp purge --before 2024-01-01 --yes
aelita-mcp forget <doc_id> --reason "stale"
aelita-mcp audit                               # inspect recent destructive actions

# Quality
aelita-mcp eval --set tests/fixtures/eval/default-queries.json
aelita-mcp eval --set <set> --baseline baseline.json
```
...
|---|---|---|
| `AELITA_MCP_WORKER_PORT` | `39888` | Worker HTTP port. |
| `AELITA_MCP_PROJECT_ID` | `default` | Project namespace (per-project vector collection). |
| `AELITA_MCP_PROJECT_ROOT` | `cwd` | Where to look for `.claude/aelita-mcp.json`. |
| `AELITA_MCP_VOYAGE_ENDPOINT` | `http://localhost:8124/v1/embeddings` | Local Voyage endpoint. |
| `AELITA_MCP_VOYAGE_MODEL` | `voyage-4-nano` | Embedding model. |
| `AELITA_MCP_VOYAGE_API_KEY` | — | Optional bearer token. |
| `AELITA_MCP_WATCH_MEMORY` | — | Comma-separated globs (channel = `memory`). |
| `AELITA_MCP_WATCH_SKILLS` | — | Comma-separated globs (channel = `skill`). |
| `AELITA_MCP_DATA_DIR` | `~/.aelita-mcp` | Storage root. |

## Per-project federation config

`<project>/.claude/aelita-mcp.json`:
...
- Rollback — drop `~/.aelita-mcp/`. claude-mem keeps working independently.
...
```bash
aelita-mcp optimize detect    # nightly OK; idempotent
aelita-mcp optimize list      # review unreviewed clusters
aelita-mcp optimize merge <cluster_id> --reason "consolidate"
```
...
```bash
aelita-mcp eval --set my-set.json --baseline baseline.json
# Exit 1 if recall@5 drops by more than 5pp vs. baseline.
aelita-mcp eval --set my-set.json --baseline baseline.json --update-baseline
```
...
```bash
export AELITA_MCP_VOYAGE_ENDPOINT=http://localhost:8124/v1/embeddings
aelita-mcp doctor
```

## doctor

`aelita-mcp doctor` checks: worker, vector DB, Voyage, federation, observation queue (Plan 2), disk usage. Each line prints PASS / WARN / FAIL with a one-line remediation.
...
beforeAll(async () => {
  workDir = mkdtempSync(join(tmpdir(), 'aelita-p3-gate-'));
...
| Voyage installer modifies `~/.aelita-mcp/` on uninstall. | Script is explicit: never touches user data. Test by inspecting the script source. |
| Federation hits leak between projects. | Per-project config lives at `<project>/.claude/aelita-mcp.json`; remotes are tagged with `source: <name>`; `chunk_id`s for remote hits use `remote:<name>:<source_uri>` prefix to avoid collisions. |
...
| Doctor reports PASS when the Voyage endpoint requires auth but no API key is set. | The Voyage probe sets the bearer header from `AELITA_MCP_VOYAGE_API_KEY` and reports FAIL on HTTP 401/403. |
...
```
~/projects/aelita-mcp/docs/plans/2026-05-07-aelita-mcp-v1-plan-3-migration-federation.md
```
